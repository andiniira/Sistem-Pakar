                            ...
                            <div class="col-md-12">
                                <div class="panel box-v4">
                                    <div class="panel-heading bg-white border-none">
                                      <h4><span class="icon-notebook icons"></span> Data Pencegahan</h4>
                                    </div>
   ...
   <div class="modal fade bs-example-modal-lg" id="tambahData" tabindex="-1" role="dialog" aria-labelledby="tambahDataLabel">
	  <div class="modal-dialog  modal-lg" role="document">
	    <div class="modal-content">
          ...
          <?php
          $cekno= mysqli_query($koneksi, "SELECT * FROM pencegahan ORDER BY kd_pencegahan DESC");
        ...
        ?>
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-4">
			      <input type="text" class="form-control" id="inputEmail3" name="kd_pencegahan" value="<?php if(isset($kodesaya)){echo $kodesaya;} else { echo "0";}  ?>" placeholder="Kode Pencegahan" readonly="readonly">
			    </div>
			  </div>
              <div class="form-group">
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-10">
			      <textarea class="form-control" placeholder="Deskripsi" name="deskripsi" required="required"></textarea>
			    </div>
			  </div>
	      </div>
...
<!-- Akhir modal tambah data -->
   ...
   <!-- Aksi Insert Data dalam DB -->
	...
	if (isset($_REQUEST['simpan'])) {
		...
		$result = mysqli_query($koneksi,"INSERT INTO pencegahan (kd_pencegahan,kode,deskripsi) 
									  values ('$kd_pencegahan','$kode','$deskripsi')");
		if ($result) {
	...
	if (isset($_POST['update'])) {
		...
		$result = mysqli_query($koneksi,"UPDATE pencegahan SET 
									  kode='$kode',
                                      deskripsi='$deskripsi'
									  WHERE kd_pencegahan='$kd_pencegahan'");
	...
	if (isset($_REQUEST['hapus'])) {
		...
		$result = mysqli_query($koneksi,"DELETE FROM pencegahan WHERE kd_pencegahan='$kd'");
		if ($result) {
 	...
 	<table class="table table-bordered table-hover">
        ...
        <td><?php echo $no; ?></td>
			...
			<td>
               ...
               <option value="">--- Pilih Penyakit --</option>
               <?php 
                    $query2="select * from penyakit order by kode";
                    ...
                    ?>
							...
							<option value="<?php echo $data2['kode'];?>"><?php echo $data2['kode'];?> - <?php echo $data2['nama_penyakit'];?></option>
						    <?php } ?>
               </select>   
                            </div>
                            <label for="inputEmail3" class="col-sm-2 control-label">Penyakit Sebelumnya</label>
						    <div class="col-sm-4">
                            <?php echo $data['kode']; ?>
                            </div>
						  </div>
						  <div class="form-group">
						    <label for="inputPassword3" class="col-sm-2 control-label">Deskripsi</label>
						    <div class="col-sm-10">
						      <textarea class="form-control" placeholder="Deskripsi" name="deskripsi" value="<?php echo $data['deskripsi']; ?>"><?php echo $data['deskripsi']; ?></textarea>
						    </div>
						  </div>
				      </div>
				      <div class="modal-footer">
                ...
                <!-- Akhir edit data -->
                <!-- Hapus data -->
				<a href="#" class="btn btn-danger btn-sm"title="Hapus Data" data-target=".bs-example-modal-lg<?php echo $data['kd_pencegahan']; ?>" data-toggle="modal"><span class="glyphicon glyphicon-trash"></span> Hapus</a>

				<div class="modal fade bs-example-modal-lg<?php echo $data['kd_pencegahan']; ?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
				  <div class="modal-dialog modal-lg" role="document">
				    <div class="modal-content">
				      <div class="modal-header">
				        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				      ...
				      <div class="modal-body">
				        ...
				        <input type="hidden" name="kode" value="<?php echo $data['kd_pencegahan']; ?>">
				      </div>
				       <div class="modal-footer">
				        <button type="button" class="btn btn-default" data-dismiss="modal">Kembali</button>
